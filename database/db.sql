...
(27,0,'Пользователи','Пользователи','#','ALL',1,1,4,NULL,NULL,'person','no','no','','yes','arrow','left'),
...
(1,1,'Преимущества способа','Преимущества способа',NULL,'Штамповка - самый недорогой способ получить множество деталей на 100% соответсвующих чертежу.\r\nДолговечность изделий.\r\nВысокая скорость, большие объемы.',1),
(2,1,'Когда вам нужна штамповка','Когда вам нужна штамповка',NULL,'Нужно быстро сделать много деталей по чертежам.\r\nУ вас есть чертежи.\r\nТолщина детали в широкой части не более 5 мм.\r\nВам нужны детали сложных форм с высокой точностью размеров.',1),
(3,1,'Наше оборудование','Наше оборудование',NULL,'Пресса кривошипные с усилием от 6 до 100 тонн.\r\nПресс ножницы комбинированные.\r\nНожницы гильотинные до 12 мм.',1);
...
/*!50003 CREATE */ /*!50017 DEFINER = 'user'@'%' */ /*!50003 TRIGGER `afterInsertAnswerQuestion` AFTER INSERT ON `answer_questions` FOR EACH ROW BEGIN
	CALL setMenuNotify();
    END */$$
...
/*!50003 CREATE */ /*!50017 DEFINER = 'user'@'%' */ /*!50003 TRIGGER `afterUpdateAnswerQuestion` AFTER UPDATE ON `answer_questions` FOR EACH ROW BEGIN
	call setMenuNotify();
    END */$$
...
/*!50003 CREATE */ /*!50017 DEFINER = 'user'@'%' */ /*!50003 TRIGGER `afterInsertCallRequest` AFTER INSERT ON `call_request` FOR EACH ROW BEGIN
	call setMenuNotify();
    END */$$
...
/*!50003 CREATE */ /*!50017 DEFINER = 'user'@'%' */ /*!50003 TRIGGER `afterUpdateCallRequest` AFTER UPDATE ON `call_request` FOR EACH ROW BEGIN
	call setMenuNotify();
    END */$$
...
/*!50003 CREATE */ /*!50017 DEFINER = 'user'@'%' */ /*!50003 TRIGGER `afterInsertContact` AFTER INSERT ON `contact` FOR EACH ROW BEGIN
	call setMenuNotify();
    END */$$
...
/*!50003 CREATE */ /*!50017 DEFINER = 'user'@'%' */ /*!50003 TRIGGER `afterUpdateContact` AFTER UPDATE ON `contact` FOR EACH ROW BEGIN
	call setMenuNotify();
    END */$$
...
/*!50003 CREATE */ /*!50017 DEFINER = 'user'@'%' */ /*!50003 TRIGGER `afterInsertOrderByDraw` AFTER INSERT ON `order_by_drawing` FOR EACH ROW BEGIN
	call setMenuNotify();
    END */$$
...
/*!50003 CREATE */ /*!50017 DEFINER = 'user'@'%' */ /*!50003 TRIGGER `afterOrderByDraw` AFTER UPDATE ON `order_by_drawing` FOR EACH ROW BEGIN
	call setMenuNotify();
    END */$$
